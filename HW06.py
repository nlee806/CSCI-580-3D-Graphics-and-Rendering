...
def scanTriangle(v0, v1, v2, normalMatrix, Cs, Kd, Ka, Ks, directional1, ambient1, n, lights, lightDirection, textures, weight,s, zBuffer):
            ...
            if ((alpha >= 0) and (beta >= 0) and (gamma >= 0)):
                ...
                if zAtPixel < zBuffer[x][y]:
                    ...
                    if s==0:
                        ...
                        RGB6 = pim6[x,y]
                        #Weight
                        aar = int(RGB1[0]*0.128+RGB2[0]*0.119+RGB3[0]*0.294+RGB4[0]*0.249+RGB5[0]*0.104+RGB6[0]*0.106)
                        aag = int(RGB1[1]*0.128+RGB2[1]*0.119+RGB3[1]*0.294+RGB4[1]*0.249+RGB5[1]*0.104+RGB6[1]*0.106)
                        aab = int(RGB1[2]*0.128+RGB2[2]*0.119+RGB3[2]*0.294+RGB4[2]*0.249+RGB5[2]*0.104+RGB6[2]*0.106)
                        im.putpixel((x,y),(aar,aag,aab))
                    zBuffer[x][y] = zAtPixel
    #TODO: Redundant?
    zBuffer = resetZBuffer()
    #print('zBuffer(should be empty):', zBuffer)
...
def toNDC(result2, sdx, sdy):
    ...
    return rasterSpace(result3, sdx, sdy)

def weights(result3, sdx, sdy):
    xwe = result3[0]+sdx
    ywe = result3[1]+sdy
    zwe = result3[2]
    result4 = [xwe, ywe, zwe]
    return rasterSpace(result4, sdx, sdy)

#raster space
def rasterSpace(result4, sdx, sdy):
    xnd = result4[0] 
    ynd = result4[1]
    znd = result4[2]
    xzero = 0
...
numSamples = 6 #num_samples (of render variations) to create 
...
for s in range(numSamples+1):
    ...
    for q in shapes:#range(len(shapes)):
        ...
        for q in range(len(data['data'])):
            ...
            #->Add [dx#,dy#]->Rasterize
            #Pixel shift(dx,dy) for each render variation (small positive or negative fractions)
            #dx,dy, weight
            #dx/(xres-1), dy/(yres-1)
            #-0.52,  0.38, 0.128,
            #0.41,  0.56, 0.119,
            ...
            #0.58, -0.55, 0.104,
            #-0.31, -0.71, 0.106
            #Weight(contribution) for each render variation(weights will sum to 1)
            weight = 0
            sdx = 0
            sdy = 0
            if s<numSamples:
                ...
                dy = aaFilter[s][1]
                weight = aaFilter[s][2]
                #print('dx, dy, weight:', dx,dy,weight)
                sdx = dx/(xres-1)
                sdy = dy/(yres-1)
                ...
                ##    vert[0] = vert[0]+sdx
                ##    vert[1] = vert[1]+sdy
                #print('Shifted vertMatrix:', vertMatrix)
            else:
                #weight = 0
                sdx = 0
                sdy = 0
                #print('No shift vertMatrix: ', vertMatrix)
            
            #World->Cam->NDC
            newC0 = worldToCam(firstCoordinateXYZ, toCam, sdx, sdy)
            newC1 = worldToCam(secondCoordinateXYZ, toCam, sdx, sdy)
            newC2 = worldToCam(thirdCoordinateXYZ, toCam, sdx, sdy)
            vertMatrix = np.array([newC0, newC1, newC2])
            #print('after NDC, vertMatrix: ', vertMatrix)
            ...
            #zBuffer = resetZBuffer()
            scanTriangle(vertMatrix[0], vertMatrix[1], vertMatrix[2], normalMatrix, Cs, Kd, Ka, Ks, directional1, ambient1, n, lights, lightDirection, textures, weight, s, zBuffer)
...
im1 = im1.save("output6a.ppm")
im2 = im2.save("output6b.ppm")
im3 = im3.save("output6c.ppm")
im4 = im4.save("output6d.ppm")
im5 = im5.save("output6e.ppm")
im6 = im6.save("output6f.ppm")
...
#Difference
imDif = Image.new('RGB', [512,512], 0x000000)

hw5ppm = Image.open("output5.ppm").convert("RGB") #HW05
hw5im = hw5ppm.load()
hw6ppm = Image.open("output6.ppm").convert("RGB") #HW06
hw6im = hw6ppm.load()
...
for y in range(height):
    for x in range(width):
        hw5rgb = hw5im[x,y]
        hw6rgb = hw6im[x,y]
        difR = abs(hw5rgb[0]-hw6rgb[0])
        difG = abs(hw5rgb[1]-hw6rgb[1])
        difB = abs(hw5rgb[2]-hw6rgb[2])
        if difR == 0 and difG == 0 and difB == 0:
            imDif.putpixel((x,y), (difR,difG,difB))
        else:
            imDif.putpixel((x,y), (255,255,255))

print("Finished.")
imDif = imDif.save("HW06difference.ppm")
